...
import { Pros, WhyShouldUse } from "~/components";
...
const ProModal: React.FC<{
  ...
  return (
    <>
      ...
      />
      <div
        className={`fixed left-2/4 top-2/4 flex -translate-x-2/4 -translate-y-2/4 flex-col items-center gap-2 rounded-smd bg-[#2B2939] ${
  ...
  return (
    <>
      ...
      <main className="flex w-full flex-col items-center gap-2">
        {!getInfo.isLoading && getInfo.data ? (
          <ProductPage
            asin={asin}
            setAsin={setAsin}
            ProductData={{
          ...
          <MainContent asin={asin} setAsin={setAsin} getInfo={getInfo as any} />
        )}
        {getInfo.error?.message.includes("You have reached your") && opened ? (
          <CreditsExpired setOpened={setOpened} />
        ) : null}
        {pro === "true" && proOpened ? (
          <ProModal setProOpened={setProOpened} />
        ) : null}
      </main>
    </>
  );
...
const MainContent: React.FC<{
  ...
  getInfo: UseTRPCMutationResult<string, Error, any, any>;
}> = ({ asin, setAsin, getInfo }) => {
  const { data: session } = useSession();
  return (
    <>
      <div
        className="relative flex h-[420px] w-full flex-col items-center gap-2 bg-[#2B293996] bg-contain bg-center bg-no-repeat"
        style={{ backgroundImage: `url("./bg2.png")` }}
      >
        <h1 className="mt-8 text-center font-Montserrat text-4xl font-bold text-white">
          Unlock the Power of Amazon Product Reviews
        </h1>
        <p className="mt-4 max-w-[40rem] text-center font-Montserrat text-2xl font-bold text-white">
          Maximize AI to analyze thousands of Amazon reviews and uncover
          game-changing insights
        </p>
        {getInfo.isLoading && session?.user ? (
          <ProgBar dataArrived={!!getInfo.data} />
        ) : (
          <div className="shadowStuff mt-14 flex w-[400px] items-center gap-2 rounded-xl bg-[#2B2939] p-2.5 lg:w-[813px]">
            <input
              className="placeholder-font-normal w-[80%] rounded-xl border-2 border-grayish bg-[#2B2939] p-2.5 font-Montserrat text-xs text-white placeholder-white focus:outline-none sm:text-sm md:text-lg"
              placeholder="Paste your Amazon ASIN"
              value={asin}
              onChange={(e) => {
                const value = e.currentTarget.value;
                // if(value.startsWith("https:")) {
                //   const asin = value.split("/")[5].split("?")[0];
                // }
                setAsin(value);
              }}
            />
            <button
              ...
              className={`shadowStuff text-whit flex h-[40px] w-[130px] items-center justify-center gap-2 rounded-lg bg-blue text-lg font-bold`}
            >
              Run Report
            </button>
          </div>
      ...
      <div
        className="flex flex-col items-center justify-center gap-8 bg-contain bg-no-repeat"
...
const ProgBar: React.FC<{
  ...
  return (
    <div className="mt-14 flex w-full flex-col items-center gap-4">
      <div className="w-[400px] rounded-full bg-[#2B2939] p-2.5 lg:w-[813px]">
        <div
          className={`${
            progress.prec === 100 && !dataArrived ? "animate-pulse" : ""
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3, 4, 5].map((star) => {
            return (
              <svg
                key={star}
                className={`h-[13px] w-[13px] sm:h-[22px] sm:w-[22px] ${
                  star <= report.stars ? "text-[#FFD37F]" : "text-grayish"
                }`}
                viewBox="0 0 24 24"
                fill="currentColor"
                xmlns="http://www.w3.org/2000/svg"
...
const CreditsExpired: React.FC<{
  ...
  return (
    <div
      className={`expLin fixed top-2/4 left-2/4 flex -translate-y-2/4 -translate-x-2/4 bg-[#2B2939] ${
        closing ? "animate-slide-out" : "animate-slide-in"
